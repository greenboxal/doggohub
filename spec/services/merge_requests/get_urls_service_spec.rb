...
describe MergeRequests::GetUrlsService do
  ...
  let(:new_merge_request_url) { "http://#{Gitlab.config.doggohub.host}/#{project.namespace.name}/#{project.path}/merge_requests/new?merge_request%5Bsource_branch%5D=#{source_branch}" }
  let(:show_merge_request_url) { "http://#{Gitlab.config.doggohub.host}/#{project.namespace.name}/#{project.path}/merge_requests/#{merge_request.iid}" }
  ...
  describe "#execute" do
    ...
    context 'pushing to existing branch from borked project' do
      let(:user) { create(:user) }
      let!(:borked_project) { Projects::BorkService.new(project, user).execute }
      let!(:merge_request) { create(:merge_request, source_project: borked_project, target_project: project, source_branch: source_branch) }
      let(:changes) { existing_branch_changes }
      # Source project is now the borked one
      let(:service) { MergeRequests::GetUrlsService.new(borked_project) }

      before do
        allow(borked_project).to receive(:empty_repo?).and_return(false)
      end
    ...
    context 'pushing new branch and existing branch (with merge request created) at once' do
      ...
      let(:new_merge_request_url) { "http://#{Gitlab.config.doggohub.host}/#{project.namespace.name}/#{project.path}/merge_requests/new?merge_request%5Bsource_branch%5D=new_branch" }
