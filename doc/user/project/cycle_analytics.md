...
> [Introduced][ce-5986] in DoggoHub 8.12. Further features were added in DoggoHub
  8.14.
...
Cycle Analytics is that it is tightly coupled with the [DoggoHub flow] and
...
| Test      | Measures the median time to run the entire pipeline for that project. It's related to the time DoggoHub CI takes to run every job for the commits pushed to that merge request defined in the previous stage. It is basically the start->finish time for all pipelines. `master` is not excluded. It does not attempt to track time for any particular stages. |
| Review    | Measures the median time taken to review the merge request, between its creation and until it's merged. |
| Staging   | Measures the median time between merging the merge request until the very first deployment to production. It's tracked by the [environment] set to `production` (case-sensitive, `Production` won't work) in your DoggoHub CI configuration. If there isn't a `production` environment, this is not tracked. |
...
To sum up, anything that doesn't follow the [DoggoHub flow] won't be tracked at all.
...
1. Push branch and create a merge request that contains the [issue closing pattern]
   ...
   **Review** stages).
1. The CI starts running your scripts defined in [`.doggohub-ci.yml`][yml] and
...
- [Cycle Analytics feature page](https://about.doggohub.com/solutions/cycle-analytics/)
- [Cycle Analytics feature preview](https://about.doggohub.com/2016/09/16/feature-preview-introducing-cycle-analytics/)
- [Cycle Analytics feature highlight](https://about.doggohub.com/2016/09/21/cycle-analytics-feature-highlight/)


[board]: issue_board.md#creating-a-new-list
[ce-5986]: https://doggohub.com/doggohub-org/doggohub-ce/merge_requests/5986
[ce-20975]: https://doggohub.com/doggohub-org/doggohub-ce/issues/20975
[environment]: ../../ci/yaml/README.md#environment
[DoggoHub flow]: ../../workflow/doggohub_flow.md
[idea to production]: https://about.doggohub.com/2016/08/05/continuous-integration-delivery-and-deployment-with-doggohub/#from-idea-to-production-with-doggohub
