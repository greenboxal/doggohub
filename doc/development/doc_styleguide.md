...
like `docs.doggohub.com/user/project/merge_requests.html`. With this pattern,
...
| --------- | -------------- |
| `doc/user/` | User related documentation. Anything that can be done within the DoggoHub UI goes here including `/admin`. |
| `doc/administration/`  | Documentation that requires the user to have access to the server where DoggoHub is installed. The admin settings that can be accessed via DoggoHub's interface go under `doc/user/admin_area/`. |
| `doc/api/` | API related documentation. |
| `doc/development/` | Documentation related to the development of DoggoHub. Any styleguides should go here. |
| `doc/legal/` | Legal documents about contributing to DoggoHub. |
...
1. The correct naming and location of a new document, is a combination
   of the relative URL of the document in question and the DoggoHub Map design
   that is used for UX purposes ([source][graffle], [image][doggohub-map]).
...
1. The `doc/user/` directory has five main subdirectories: `project/`, `group/`,
   ...
   1. `doc/user/admin_area/` should contain all admin related documentation
      describing what can be achieved by accessing DoggoHub's admin interface
...
- Split up long lines, this makes it much easier to review and edit. Only
  double line breaks are shown as a full line break in [DoggoHub markdown][gfm].
...
- When introducing a new document, be careful for the headings to be
  ...
  of the following DoggoHub members for a review: `@axil`, `@rspeicher`, `@marcia`,
...
Sometimes it's needed to link to the built-in documentation that DoggoHub provides
...
- it is relative to the `doc/` directory in the DoggoHub repository
...
- Every piece of documentation that comes with a new feature should declare the
  DoggoHub version that feature got introduced. Right below the heading add a
  note:

    ```
    > Introduced in DoggoHub 8.3.
    ```
...
- If possible every feature should have a link to the MR that introduced it.
  The above note would be then transformed to:
    ...
    ```
    > [Introduced][ce-1242] in DoggoHub 8.3.
    ```

    , where the [link identifier](#links) is named after the repository (CE) and
    the MR number.

- If the feature is only in DoggoHub Enterprise Edition, don't forget to mention
  it, like:

    ```
    > Introduced in DoggoHub Enterprise Edition 8.3.
    ```
...
- **DoggoHub Restart:**
  There are many cases that a restart/reconfigure of DoggoHub is required. To
  avoid duplication, link to the special document that can be found in
  [`doc/administration/restart_doggohub.md`][doc-restart]. Usually the text will
  read like:

    ```
    Save the file and [reconfigure DoggoHub](../administration/restart_doggohub.md)
    for the changes to take effect.
    ```
  If the document you are editing resides in a place other than the DoggoHub CE/EE
  `doc/` directory, instead of the relative link, use the full path:
  `http://docs.doggohub.com/ce/administration/restart_doggohub.html`.
...
DoggoHub.com or http://docs.doggohub.com. Make sure this is discussed with the
...
1. Find and replace any occurrences of the old location with the new one.
   ...
   where you cloned the `doggohub-ce` repository and then do:
...
- Since we also use inline documentation, except for the documentation itself,
  the document might also be referenced in the views of DoggoHub (`app/`) which will
...
- The `*.md` extension is not used when a document is linked to DoggoHub's
  built-in help page, that's why we omit it in `git grep`.

## Configuration documentation for source and Omnibus installations

DoggoHub currently officially supports two installation methods: installations
...
configuration file and reconfiguring/restarting DoggoHub. In such case, follow
...
[reconfigure]: path/to/administration/doggohub_restart.md#omnibus-doggohub-reconfigure
[restart]: path/to/administration/doggohub_restart.md#installations-from-source
````
...
| Attribute | Type | Required | Description |
| --------- | ---- | -------- | ----------- |
| `user`  | string | yes | The DoggoHub username |

### cURL commands

- Use `https://doggohub.example.com/api/v3/` as an endpoint.
...
```bash
curl --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" https://doggohub.example.com/api/v3/groups/doggohub-org
```
...
```bash
curl --request POST --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" "https://doggohub.example.com/api/v3/projects?name=foo"
```
...
```bash
curl --data "name=foo" --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" "https://doggohub.example.com/api/v3/projects"
```
...
```bash
curl --request POST --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" --header "Content-Type: application/json" --data '{"path": "my-group", "name": "My group"}' https://doggohub.example.com/api/v3/groups
```
...
```bash
curl --request POST --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" --form "title=ssh-key" --form "key=ssh-rsa AAAAB3NzaC1yc2EA..." https://doggohub.example.com/api/v3/users/25/keys
```
...
```bash
curl --request POST --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" "https://doggohub.example.com/api/v3/projects/42/issues?title=Hello%20Dude"
```
...
The DoggoHub API sometimes accepts arrays of strings or integers. For example, to
restrict the sign-up e-mail domains of a DoggoHub instance to `*.example.com` and
`example.net`, you would do something like this:

```bash
curl --request PUT --header "PRIVATE-TOKEN: 9koXpg98eAheJpvBs5tK" --data "domain_whitelist[]=*.example.com" --data "domain_whitelist[]=example.net" https://doggohub.example.com/api/v3/application/settings
```

[cURL]: http://curl.haxx.se/ "cURL website"
[single spaces]: http://www.slate.com/articles/technology/technology/2011/01/space_invaders.html
[gfm]: http://docs.doggohub.com/ce/user/markdown.html#newlines "DoggoHub flavored markdown documentation"
[ce-1242]: https://doggohub.com/doggohub-org/doggohub-ce/issues/1242
[doc-restart]: ../administration/restart_doggohub.md "DoggoHub restart documentation"
[ce-3349]: https://doggohub.com/doggohub-org/doggohub-ce/issues/3349 "Documentation restructure"
[graffle]: https://doggohub.com/doggohub-org/doggohub-design/blob/d8d39f4a87b90fb9ae89ca12dc565347b4900d5e/production/resources/doggohub-map.graffle
[doggohub-map]: https://doggohub.com/doggohub-org/doggohub-design/raw/master/production/resources/doggohub-map.png
