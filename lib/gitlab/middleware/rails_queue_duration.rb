...
# doggohub-workhorse forwarding a request to the Rails application and the
  ...
  module Middleware
    class RailsQueueDuration
      ...
      def call(env)
        ...
        proxy_start = env['HTTP_DOGGOHUB_WORKHORSE_PROXY_START'].presence
        if trans && proxy_start
          # Time in milliseconds since doggohub-workhorse started the request
