...
namespace :spinach do
  namespace :project do
    desc "DoggoHub | Spinach | Run project commits, issues and merge requests spinach features"
    task :half do
      run_spinach_tests('@project_commits,@project_issues,@project_merge_requests')
    end

    desc "DoggoHub | Spinach | Run remaining project spinach features"
    task :rest do
      run_spinach_tests('~@admin,~@dashboard,~@profile,~@public,~@snippets,~@project_commits,~@project_issues,~@project_merge_requests')
    end
  end

  desc "DoggoHub | Spinach | Run project spinach features"
  task :project do
    run_spinach_tests('~@admin,~@dashboard,~@profile,~@public,~@snippets')
  end

  desc "DoggoHub | Spinach | Run other spinach features"
  task :other do
    run_spinach_tests('@admin,@dashboard,@profile,@public,@snippets')
  end
